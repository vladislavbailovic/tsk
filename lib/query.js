...
var QueryFilters = {
	...
	and: function (filters) {
		return function (data, context) {
			return filters.every(QueryFilters.matches(data));
		};
	},
	...
	or: function (filters) {
		return function (data, context) {
			return filters.some(QueryFilters.matches(data));
		};
	},
	...
	andor: function (andf, orf) {
		return function (data, context) {
			return andf.every(QueryFilters.matches(data)) || orf.some(QueryFilters.matches(data));
		};
	},
	 ...
	 *
	 * @param {String} data Data context to check against
	 *
	 * @return {Function} Function accepting a single regex/string parameter to match in data
	 */
	matches: function (data) {
		return function (query_rx) { return !!query_rx && data.match(query_rx); };
	}
};
